...
public class JComboBoxDecorator {
	...
	private List<StringSpecies> previousEntries = new ArrayList<StringSpecies>();
	...
	private void comboFilter(String enteredText, List entries) {
		...
		if (enteredText == null) {
			...
			if (species)
				entriesFiltered = StringSpecies.getModelSpecies();
			else
			...
			if (species)
				entriesFiltered = StringSpecies.search(enteredText);
			else
	...
	private List entrySearch(String enteredText) {
		...
		for (Object o : allEntries) {
			if (o.toString().toUpperCase().startsWith(enteredText)) {
				StringSpecies sp = getSpecies(o.toString());
				if (sp != null)
					ret.add(sp.toString());
			}
		}
	...
	private StringSpecies getSpecies(String speciesName) {
		if (nameSpecies == null || speciesName == null)
			return null;
